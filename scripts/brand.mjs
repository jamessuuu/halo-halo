 ...
 * showcase-program/brand/brand.mjs and extended with halo-halo's own glyph,
 * same pattern clarifier used (CLAUDE.md: "brand.mjs ... extended with
 * clarifier's own glyph"). The palette, grid, stroke weights and the
 * dot-matrix face all come from agentjames/scripts/branding/system.mjs
 * (docs/DESIGN.md is the binding contract). The chip mark is the maker's
 * mark and is IDENTICAL everywhere; only the project glyph differs.
 *
 ...
 * diff the output and fail on drift (pnpm ci:brand-check).
 *
 * Usage:  node scripts/brand.mjs --project=halo-halo --out=<dir>
 */
...
// the project names need. "halo-halo" needs h,a,l,o only — the hyphen is
// simply not drawn (word() skips any character with no FONT entry and still
// advances the cursor), which reads as a deliberate word-gap, not a bug.
// ---------------------------------------------------------------------------
const FONT = {
...
const GLYPHS = {
  /**
   * halo-halo — the mechanism, not the dessert: two ordinary same-language
   * tokens (small, solid, unremarkable — squat ink squares grounded on a
   * shared baseline) beside the one word that actually matters: a single
   * orthographic span standing taller, split into an affix (solid ink) and
   * a root (open ink outline — the open-class side, the one with a real
   * OOV recall gap per docs/limitations), with the split itself drawn as
   * the ONE amber element. That amber bar is the whole claim: not a color
   * for "Tagalog" or "English," but the literal intra-word switch point —
   * "nag-|book" — which is what UAX #29 alone cannot see (SPEC.md
   * Architecture §1) and what Tier 2 exists to draw.
   */
  "halo-halo": () =>
    [
      // two ordinary tokens — plain, single-language, nothing to see here
      `<rect x="8" y="42" width="8" height="8" fill="${INK}"/>`,
      `<rect x="18" y="42" width="8" height="8" fill="${INK}"/>`,
      // the word that matters: affix (solid) | split (amber) | root (outline)
      `<rect x="30" y="14" width="10" height="36" fill="${INK}"/>`,
      `<rect x="40" y="14" width="4" height="36" fill="${AMBER}"/>`,
      `<rect x="44" y="14" width="14" height="36" fill="none" stroke="${INK}" stroke-width="${SW}"/>`,
    ].join(""),
...
const COMPACT = {
  /**
   * halo-halo — a short solid ordinary-token square grounds the reading,
   * then the split-word as one strong tri-band silhouette: ink (affix),
   * amber (the split — wide enough to survive at 16px, unlike a hairline),
   * ink (root). Distinguishable from every sibling favicon: none of them
   * are a horizontal three-band composition (sluice = overhead gate,
   * snapgauge = stacked horizontal jaws, chaff = grid + corner square,
   * tiltmeter = diagonal + square, dogwatch = bell silhouette).
   */
  "halo-halo": () =>
    [
      // small ordinary token, low and to the left
      `<rect x="6" y="46" width="12" height="12" fill="${INK}"/>`,
      // the split word: affix | split | root, tall and solid
      `<rect x="24" y="10" width="12" height="40" fill="${INK}"/>`,
      `<rect x="36" y="10" width="8" height="40" fill="${AMBER}"/>`,
      `<rect x="44" y="10" width="14" height="40" fill="${INK}"/>`,
    ].join(""),
...
async function main() {
  ...
  // ── Icon family ───────────────────────────────────────────────────────────
  // The PROJECT's own glyph is the identity in a tab, not the chip: identical
  // chip favicons are indistinguishable in a tab strip, which is the one job
  // a favicon has. The chip stays the maker's mark in the footer and the
  ...
  // compact drawing at each size by the PNG step below.
  ...
  const cell = 3.2;
  const w = word(project.replaceAll("-", ""), 84, 30, cell, INK);
  ...
  const ogWord = word(project.replaceAll("-", ""), 96, 250, 12, INK);
  put(
    "og.svg",
    svg(
      1200,
